...
restore_cache() {
  ...
  archive="$(mktemp)"
  headers="$(mktemp)"
  trap 'rm -f "$archive" "$headers"' RETURN
  start="$(_millis)"
  http_code="$(curl_with_retry -sS --max-filesize "$MAX_COMPRESSED_BYTES" \
    -D "$headers" -o "$archive" -w '%{http_code}' \
    -H "Authorization: Bearer $CACHE_TOKEN" "$CACHE_API/v2/cache/$CACHE_KEY")" || http_code=000
  case "$http_code" in
    404)
  ...
  [[ "$expected" =~ ^[0-9a-f]{64}$ ]] || { echo "r2-cache-v2: missing or invalid checksum metadata" >&2; return 1; }
  actual="$(sha256_file "$archive")"
  [[ "$actual" == "$expected" ]] || { echo "r2-cache-v2: checksum mismatch" >&2; return 1; }
  python3 "$ACTION_PATH/cache-v2-archive.py" restore \
